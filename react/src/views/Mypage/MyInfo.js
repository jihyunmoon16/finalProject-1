...
function MyInfo() {
    ...
    async function getUserInfo(nickname) {
        await axios.get("/api/user/info", { params: { nickname: nickname } })
            .then((response) => {
    ...
    async function checkNickname(event) {
        ...
        await axios.get("/api/user/check", { params: { nickname: nickname } })
            .then((response) => {
    ...
    async function chodaeLeave() {
        ...
        if (result) {
            // console.log("회원탈퇴");
            await axios.delete(`/api/user/bye/${nickname}`)
                .then((response) => {
                    console.log(response);
                    store.logout();
                    navigate("/");
            ...
            <div className={styles.boardContainer}>
                ...
                <div>
                    ...
                    <input
                        ...
                        disabled
                    ></input>
                    {onUpdate ? <button className={styles.checkBtn} onClick={checkNickname}>중복검사</button> : null}
                    {!isValidNickname &&
                        <span className={styles.errorMessage}>
                            {nicknameMsg.length > 0 && <AiOutlineWarning className={styles.msgIcon} />}{nicknameMsg}
                        </span>}
                    {isValidNickname &&
                        <span className={styles.okMessage}>
                            {nicknameMsg.length > 0 && <AiOutlineCheckCircle className={styles.msgIcon} />}{nicknameMsg}
                        </span>}
                </div>
                <div className={styles.comBox}>
                    {!onUpdate ?
                        <button className={styles.checkBtn} onClick={fieldsetWritable}>수정하기</button> :
                        <button className={styles.checkBtn} onClick={fieldsetDisable}>수정완료</button>
                    }
                    <button className={styles.checkBtn} onClick={chodaeLeave}>회원 탈퇴</button> :
                </div>
